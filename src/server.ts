...
interface ServerProps {
  ...
  clientApplicationLoadBalancer: ApplicationLoadBalancer;
  sshPubKey: string;
...
export class ServerResources extends Construct {
  ...
  constructor(scope: Construct, id: string, props: ServerProps) {
      ...
      init: CloudFormationInit.fromConfigSets({
        ...
        configs: {
          ...
          downloads: new InitConfig([
            InitCommand.shellCommand('mkdir -p /var/lib/asterisk/sounds'),
            InitCommand.shellCommand(
              'aws s3 cp s3://' +
                assetBucket.bucketName +
                '/audio /var/lib/asterisk/sounds --recursive',
            ),
          ...
          config: new InitConfig([
            ...
            InitFile.fromString(
              ...
              `SIP_URI=sip:${phoneNumber}@${
                props.distribution.domainName
              }\nSIP_PASSWORD=${
                Stack.of(this).artifactId
              }\nWEBSOCKET_URL=wss://${
                props.distribution.domainName
              }/ws\nVOICE_CONNECTOR_PHONE=${phoneNumber}\nSERVER_IP=${
                props.serverEip.ref
            ...
            InitCommand.shellCommand('apt remove libopusfile0 -y'),
          ]),
        },
      }),
      initOptions: {
    ...
    const httpListener = props.clientApplicationLoadBalancer.addListener(
      'httpListener',
      {
        port: 80,
        protocol: ApplicationProtocol.HTTP,
    ...
    const webSocketListener = props.clientApplicationLoadBalancer.addListener(
      'webSocketListener',
      {
        port: 8088,
        protocol: ApplicationProtocol.HTTP,
    ...
    ec2InstanceSecurityGroup.connections.allowFrom(
      new Connections({
        securityGroups: [props.clientAlbSecurityGroup],
      }),
      Port.tcp(8088),
